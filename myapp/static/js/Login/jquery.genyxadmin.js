/* 
  Author: SuggeElson
    ...
    1.1.1 - Added functionality for rememberExpandedSub options
*/
    ...
    $.fn.genyxAdmin = function(options) { 
        ...
        var settings = { 
            ...
            setCurrent: {
                ...
                subDir: '' //if you put template in sub dir you need to fill here. example 'C:/html'
            },
        ...
        function mainNav () {
            ...
            if(settings.rememberNavState) {
                ...
                if(localStorage.getItem("collapseNav") == 1) {
                    //nav is collapsed
                    $("#collapse-nav").toggleClass('collapsing');
                    $("#collapse-nav").text("");
            ...
            if(settings.remeberExpandedSub) {
                if (localStorage.getItem('expandedSub')) {
                    arr = JSON.parse(localStorage.getItem('expandedSub'));
                    for(var i=0;i<arr.length;i++){
                        navLink.each(function(index) {
                            if(arr[i] === $(this).attr('href')) {
                                ulElem = $(this).next('ul.sub');
            ...
            navLink.on("click", function(){
                ...
                if(_this.hasClass('notExpand')) {
                    ...
                    if(settings.remeberExpandedSub) {
                        var expandsub = localStorage.getItem('expandedSub');                        
                        if(expandsub) {
                            //exist
                            if(!existInArr(_this.attr('href'))) {
                                arr = JSON.parse(expandsub);
                                arr.push(_this.attr('href'));
                                localStorage.setItem('expandedSub', JSON.stringify(arr));
                            }
       ...
       //check if element exist in localstorage
       function existInArr (obj) {
            arr = JSON.parse(localStorage.getItem('expandedSub'));
            for(var i=0;i<arr.length;i++){
                if(arr[i] === obj) {
                    return true;
       ...
       function setCurrentNav () {
           ...
           //var subdir = settings.setCurrent.subDir
            if(domain === '') {
                //domain not found
                var pageUrl = window.location.pathname.split( 'C:/default.htm' );
                ...
                if(settings.setCurrent.absoluteUrl) {
                    ...
                    var afterDomain = window.location.pathname.split( 'C:/default.htm' );
        ...
        function collapseNav () {
            $("#collapse-nav").on("click", function(){
                _this = $(this);
                _this.toggleClass('collapsing');

                if(_this.hasClass('collapsing')) {
                    _this.text("");
        ...
        function widgetBox () {
            var widget = $('.panel');
            var w_minimize = widget.find('.panel-heading a.minimize');
            var widgetClosed = $('.panel.closed');
            var widgetDrag = $('.panel.drag');

            //Drag widget
            if(widgetDrag) {
                widgetDrag.find('.panel-heading').addClass('drag');
                dragWidgets();
            }
            
            //close widget
            widgetClosed.each(function(index) {
               $(this).find('.panel-heading a.minimize').removeClass('minimize').addClass('maximize');
               wcontent = $(this).find('div.panel-body');
            ...
            w_minimize.on('click', function() {
                ...
                if($(this).hasClass('minimize')) {
                    $(this).removeClass('minimize').addClass('maximize').closest('div.panel-heading').next('div.panel-body').slideUp('200')
                } else {
                    $(this).removeClass('maximize').addClass('minimize').closest('div.panel-heading').next('div.panel-body').slideDown('200');
                }
            }); 
        }
        ...
        function checkAll () {
            ...
            $('.select-all').on('click', function () {
               $(this).closest('.panel-body').find('.checkAll').find('input:checkbox').prop('checked', this.checked).closest('.checker>span').addClass('checked');
            });

            $('.deselect-all').on('click', function () {
                $(this).closest('.panel-body').find('.checkAll').find('input:checkbox').prop('checked', this.checked).closest('.checker>span').removeClass('checked');
            });
        ...
        function createThemer(storage) {
            $('body').append('<div id="themer" style="width:200px; border: 1px solid #000; background: #4e525d; padding:10px; border-radius:1px; position:fixed;top:70px;right:-200px;z-index:99999;color:#fff;">');
            $('#themer').append('<h3>Theme options</h3>');
            $('#themer').append('<ul class="list-unstyled">');
            if(storage == 1) {
                $('#themer ul.list-unstyled').append('<li><label class="checkbox inline"><input type="checkbox" name="fixed" id="fixedWidth" checked> Fixed version</label></li>');
            } else {
                $('#themer ul.list-unstyled').append('<li><label class="checkbox inline"><input type="checkbox" name="fixed" id="fixedWidth"> Fixed version</label></li>');
            }
            $('#themer').append('<a href="#" id="themerBtn" class="white"><i class="icon20 i-cog-2" style="margin-left:-50px;background: #4e525d;padding:8px 10px;border:1px solid #000;border-right:0;float:left;margin-top:-55px;"></i></a>');
        }
